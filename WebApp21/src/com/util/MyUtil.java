...
{
	// ■ 전체 페이지 수를 구하는 메소드
	// numberPage : 한 페이지에 표시할 데이터(게시물) 의 수
	// dataCount  : 전체 데이터(게시물) 수
	// 한 페이지에 몇 개인지 몇 개를 뿌릴건지
	...
	//   『32 / 10』 의 연산은 수행하여 결과 3을 얻을 수 있다.
	//   → 『pageCount = dataCount / numPerPage;』
	...
	public String getIndexList(int currentPage, int totalPage, String listUrl)
	{
		...
		*/
		...
		if(listUrl.indexOf("?") != -1)		// 링크 설정할 URL에 『?』 가 들어가 있으면
		{
			listUrl = listUrl + "&";
		}
		else								// 링크 설정할 URL에 『?』 가 들어가 있지 않으면
		{
			listUrl = listUrl + "?";
		}
		...
		//   『20 / 10 = 2』 이며... 여기에 『* 10』(10을 곱해도) 20이 되는데
		//   이런 상황이라면 다시 10을 빼서 10으로 만들어준다.
		//   맨 마지막 페이지 처리 때문에 추가 처리를 해줘야 한다.
		...
		// Next
		n = currentPage + numPerBlock;
		if ( (totalPage - numPerBlock) > numPerBlock ) 
